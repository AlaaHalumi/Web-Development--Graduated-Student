<?php
	...
	$queryId = "SELECT * FROM `tbl_formData_215` WHERE id=".$id;
	$result = mysqli_query($connection, $queryId);
	$row = $result->fetch_assoc();
		
	
	
 	$target_file_profile = $target_dir_profile .$row["mainID"]. "profile.png"; 
	$target_file_standpicture = $target_dir_standpicture . $row["mainID"]. "standpicture.png";
	$target_file_projectpicture1 = $target_dir_projectpicture . $row["mainID"]. "projectpicture1.jpg";
	$target_file_projectpicture2 = $target_dir_projectpicture . $row["mainID"]. "projectpicture2.jpg";
	$target_file_projectpicture3 = $target_dir_projectpicture . $row["mainID"]. "projectpicture3.jpg";
	$target_file_cv = $target_dir_cvfile .$row["mainID"]. "cv.pdf"; 
	...
	$picture_exist[4],$cv_upload,$row["mainID"]);
